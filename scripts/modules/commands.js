...
function registerCommands(programInstance) {
	...
	programInstance
		...
		.option(
			'--burncloud',
			'Allow setting a Burncloud model ID (use with --set-*) '
		)
		.addHelpText(
			'after',
			`
Examples:
  ...
  $ task-master models --set-main claude-sonnet-4-20250514 --burncloud # Set Burncloud model for main role
  $ task-master models --setup                            # Run interactive setup`
		...
		.action(async (options) => {
			...
			if (providerFlags > 1) {
				console.error(
					chalk.red(
						'Error: Cannot use multiple provider flags (--openrouter, --ollama, --bedrock, --burncloud) simultaneously.'
					)
				);
				process.exit(1);
			}
